...
## Project specifics: calx-telltale

The spine above is the template's, copied unchanged at the adopted revision.
This section holds what is true of this project alone, and it contradicts
nothing above it.

### The software

calx-telltale is a Rust crate. The build is `cargo build --release --locked`,
and the proofs are `cargo kani`. The arithmetic is the verified part, and the
...
Every result-bearing surface reports the provenance the result carries and the
witness that attains it. Provenance is part of the data rather than part of the
prose, so it reaches the structured output and the human report alike. This is
set in `call/0003` and it is the property the tool is named for. A change that
lets a cost reach either surface without its provenance is a defect, whatever
else it improves.

### Prose

Authored prose here is British English. The `host-lint` prose lane runs over it
like any other host, and `MEMORY.md` is excluded through `.host-lintignore` as
the append-only record.

Acceptance criteria in `plan/` are stated as properties rather than as expected
numbers. The worked figures that would otherwise supply those numbers come from
an investigation under non-disclosure, and `call/0002` records why they stay out
of this repository.

### The Where room

`calx-telltale` is greenfield, so its `.host-software` stanza carries no `build`
and no `artifact` until there is a binary to re-derive. Adding them, with a
pinned toolchain and a recorded hash, is part of the first milestone that
produces one.
